...
//Wybór płci
let genderIndex = 0;
Genders.forEach(gender => gender.addEventListener("click", function (e) {
    e.preventDefault();
...
//Wybór sylwetki
let bodyTypeIndex = 0;
bodyTypes.forEach(type => type.addEventListener("click", function (e) {
...
const CountCalories = function (e) {
    ...
    if (AgeInput.value === "" || WeightInput.value === "" || TallInput.value === "" || AgeInput.value <= 0 || WeightInput.value <= 0 || TallInput.value <= 0 || genderIndex === 0 || bodyTypeIndex === 0) {
        ...
        setTimeout(function () {
            const scoreDescriptions = [...document.querySelectorAll(".result__scoreDescription")];
            scoreDescriptions.forEach(scoreDescription => scoreDescription.classList.add("active"));
        }, 2000);
    }
